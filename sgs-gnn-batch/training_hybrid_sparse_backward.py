...
# Local experimental variant of the hybrid trainer.
# The default `--implementation github` path uses `training_hybrid.py` to match upstream.
    ...
    for batch in cluster_loader:
        ...
        if mode == 'learned':
            ...
            if batch.edge_index.shape[1] > q:                                          
                ...
                # pass 1: edge probability for sampling (full graph)
                if use_checkpoint:
                    with torch.no_grad():
                ...
                edge_probs_full_1d = edge_probs_full.squeeze()
                ...
                if use_checkpoint:
                    edge_probs_full = model.edge_prob_mlp.forward_sparse_backward(
                        batch.x,
                        batch.edge_index,
                        sampled_edge_indices,
                        random_sampled_edge_index=random_sampled_edge_index,
                        use_checkpoint=True,
                        edge_probs_full=edge_probs_full,
                    )
                    edge_probs_sampled = edge_probs_full.squeeze()[sampled_edge_indices]
                else:
                    edge_probs_sampled = edge_probs_full_1d[sampled_edge_indices]
